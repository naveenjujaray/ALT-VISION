...
public class CameraSource {
    ...
    public CameraSource(MainActivity activity, GraphicOverlay overlay) {
    ...
    private void updateData(byte[] d) {
        this.data = d;
    }
    ...
    private class FrameProcessingRunnable implements Runnable {
         ...
         * present) restart to the camera, and keeps a pending reference to the frame data for future use.
         */
        ...
        public void run() {
            ...
            while (true) {
                synchronized (lock) {
                    ...
                    // recycled restart to the camera before we are done using that data.
                ...
                try {
                    synchronized (processorLock) {
                        ...
                        if(touch && searching) {
                            //only processes when there is a touch on the screen allows
                            // for processing to be efficient
                            frameProcessor.process(
                                    ...
                                    new FrameMetadata.Builder()
                                            ...
                                            .build(),
                                    graphicOverlay);
                            CameraSource.this.width = previewSize.getWidth();
                            CameraSource.this.height = previewSize.getHeight();
                        }
                    }
                } catch (Throwable t) {
    ...
    public int zoom(int level){
        Camera.Parameters param = camera.getParameters();
        int zoom;
        if(param.isZoomSupported() && level == 1) {
            zoom = param.getMaxZoom()/4;
        }
        else if(param.isZoomSupported() && level == 2) {
            zoom = param.getMaxZoom()/2;
        }
        else if(param.isZoomSupported() && level == 3) {
